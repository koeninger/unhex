...
Cover applies only to attacks made by characters from other areas. This means it does not apply to melee attacks, nor to ranged attacks within the same area. A target <<combat.adoc#intercept, intercepted>> while moving between areas does not benefit from cover.
...
If a character is <<combat.adoc#intercept, intercepted>> while moving between areas, use the smaller range. A character moving between an area at range one and zero is effectively at range zero, between two and one is at range one, and so on.
